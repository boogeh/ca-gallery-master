...
function getAllPossibleCoordsQueen(pieceCoord) {
    var res = []
    var rookPoss = getAllPossibleCoordsRook(pieceCoord)
    var bishopPoss = getAllPossibleCoordsBishop(pieceCoord)
    for (var i = 0; i < rookPoss.length; i++) {
        res.push(rookPoss[i])
    }
    for (i = 0; i < bishopPoss.length; i++) {
        res.push(bishopPoss[i])
    }
